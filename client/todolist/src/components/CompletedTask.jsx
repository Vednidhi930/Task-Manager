...
const getData=async()=>{
  try {
    ...
    setCompleteTask(response.data.taskData.tasks)
  } catch (error) {
  ...
  return (
    <>
       <div className="my-2 relative  items-center w-full h-12">
      <motion.h1 initial={{x:-100,opacity:0}} animate={{ x:0,opacity:1 }}transition={{ease:"easeInOut",duration:0.5}} className="text-2xl my-2 ms-2 p-1 ">Completed Tasks</motion.h1>
      </div>


   {!completedTask.length ?   <motion.div 
   initial={{x:-100,opacity:0}} animate={{ x:0,opacity:1 }}transition={{ease:"easeInOut",duration:0.5}}
   className="w-full  flex flex-col justify-center items-center mt-10 h-[100vh]">
          <FcDoNotInsert  className="text-[20rem]"/>
          <h1 className="text-[2rem]">No Completed Task</h1>
      </motion.div> :
   <div className="w-full h-[100vh] flex flex-wrap">
      {!completedTask ? null:completedTask.map((curr, id) => (
          <motion.div
